...
class Exp_Forecast(Exp_Basic):
    ...
    def vali(self, vali_data, vali_loader, criterion, is_test=False):
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(vali_loader):
                ...
                outputs,_ = self.model(batch_x, batch_x_mark,None, batch_y_mark, None)
                if is_test:
    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(train_loader):
                ...
                outputs,L_aux = self.model(batch_x, batch_x_mark, None, batch_y_mark, None)
                if self.args.dp:
                ...
                if self.args.covariate:
                    if self.args.last_token:
                        ...
                        outputs = outputs[:, :, -1]
                        batch_y = batch_y[:, :, -1]
                loss = criterion(outputs, batch_y)+L_aux
                if (i + 1) % 100 == 0:
                    if (self.args.ddp and self.args.local_rank == 0) or not self.args.ddp:
                        print("\titers: {0}, epoch: {1} | loss: {2:.7f} | aux_loss: {3:.7f}".format(i + 1, epoch + 1, loss.item(),L_aux))
                        speed = (time.time() - time_now) / iter_count
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_loader):
                ...
                if not self.args.nonautoregressive:
                    ...
                    outputs,_= self.model(batch_x, batch_x_mark, None, batch_y_mark, None)
                    pred_y = outputs[:, -self.args.test_pred_len:, :]
                batch_y = batch_y[:, -self.args.test_pred_len:, :].to(self.device)
    ...
    def count_parameters(self,model):
        ...
        print(f"Total trainable parameters: {num_param/1000000000}B")
